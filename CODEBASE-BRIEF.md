...
```
`reason` ∈ `included, outlier_high, outlier_low, pinned, not_a_reel, missing_views, beyond_top_6` (last one legacy-pipeline only). `sponsored` and `collab` are legacy values that still appear in old stored reports; new reports never emit them. Each candidate instead carries `isSponsored` / `isCollab` booleans so the UI can label those posts (they are counted in the averages). The profile results column formerly headed "Reels Skipped" is now "Not Counted" (pinned, non-Reel, missing views, outlier trim).
...
- **Profile reel selection — legacy** (`selectProfileReels`): exclude pinned and non-Reel posts (sponsored and collab posts now COUNT, changed Sep 2026 on client request); sort remaining by timestamp descending (explicit fix — actor return order is not reliably chronological); compute median views; two-sided outlier exclusion at `median × 3` (`OUTLIER_MULTIPLIER`, both directions, toggleable); take the most-recent 6 "normal" reels; if fewer than 6 remain, backfill from excluded outliers closest to the median.
- **Profile reel selection — Express/V2** (`selectProfileReelsV2`): same pinned-only exclusion (sponsored/collab count); sorts by view count in **log space** (view counts are ~log-normal) and trims the top/bottom `PROFILE_V2_TRIM_PCT` (default 15%) instead of hard-excluding past a multiplier; `avgViews` is then also computed as a **log-space mean**, not a linear average. Degrades more gracefully than legacy at small fetch depths.
...
**Profile actor gotchas (Sep 2026)**: `instagram-profile-reels-scraper` renamed `owner` to `user` (`followers` to `follower_count`); `normalizeProfileReelItemV2` reads either, and `scrapeProfilesBatchV2` logs an error if posts come back with no usable username (schema drift used to surface only as "Instagram returned no data" on every profile). The actor also rejects `postsPerProfile < 5` with HTTP 400, so the admin fetch depth minimum is 5. Cost is linear in results returned ($0.001 each plus $0.0005 per run of 5 profiles).

**Help assistant (static, no outside service)**: floating chat bubble (`client/src/components/HelpBot.jsx`, styles in `styles/helpbot.css`, mounted once in `App.jsx`). All knowledge is hand-written in `client/src/content/helpKnowledge.js` (about 90 entries plus small talk and page-navigation targets); `client/src/utils/helpMatcher.js` does the understanding locally (normalizing, synonym folding, typo repair against our own vocabulary, idf-weighted scoring, page and follow-up context, two-part questions, "take me to X" navigation). Answers can be functions of the user (real credits, plan, which premium features are unlocked). Open to everyone including logged-out visitors on marketing pages; hidden on auth, checkout, admin, and client-facing pages (`shouldShow`). Feedback loop: `POST /api/help/log` (public, rate limited, `server/routes/help.routes.js`) stores only unanswered question text (emails, links and long digit runs masked) and thumbs votes, in `helpEvents` with a 120 day TTL; admins read it at `/admin/help` and add the missing answer to `helpKnowledge.js`. **Confidentiality rule**: `tests/helpbot.test.js` fails the run if any answer mentions the data vendor, costs, margins, pipelines, or currency amounts; prices are never quoted, the bot points to the Pricing page. The PDF answer follows `features.pdfExport` (built but no plan grants it yet), so it never promises what the account cannot do. Planned later: an optional LLM fallback for unmatched questions (send only the question plus matched knowledge text, never customer report data).

**Guided tour display rules (DemoGuide.jsx)**: what is on screen is always one measured snapshot (step + highlight + card) and it only appears when the page has finished loading (no `.rl-loader-mark` or `.rl-skel` on screen for 160ms, capped at 8s so a stuck spinner cannot hide it). Changing steps on the same page keeps the old snapshot until the next is steady, then the highlight glides; changing pages fades the tour out on click and back in when the new page settles. Measuring is per animation frame until steady. Later steps' pages are warmed one at a time only while a card is being read (starting them earlier competed with the page being loaded on slow connections). Escape ends the tour, and the help bubble hides while it runs (`data-tour-active` on the root element).

**Team members and data ownership (rule of thumb)**: anything a member creates or reads belongs to the workspace owner, so use `req.currentUser.effectiveUsername`, never `.username`, for data. Two places were missed at first and only found by signing in as a real member: the sample report (`POST/DELETE /jobs/demo`) and the owner-rename cascade in `PATCH /auth/username` (which must also update members' `teamOwnerUsername`, `teamInvites`, `analyzedCreators`). `tests/team.test.js` covers both. Paused reports record `pausedAt` so Elapsed stops counting at the pause (older paused reports fall back to `updatedAt`).

**No unlimited credits, anywhere (Sep 2026)**: nobody has an unlimited pool, admin included. An admin's balance is not stored (stored value is 0, plan `admin`, normalised on every boot by `backfillCredits`); it is what Apify's remaining monthly allowance can fund, computed live by `server/services/platformCredits.service.js` and applied in `middleware/auth.js` and `publicUser`. The formula: left to spend = allowance minus used; cost per credit = measured cost per item divided by credits per item (Reel 1, profile 5); credits = left divided by the dearer cost per credit, so it is covered whatever mix runs. `GET /admin/platform-credits` returns every step; the admin sees it as a table at the top of Usage & Spend and as the "Apify Credits" sidebar card. Pre-flight credit checks therefore bind admin too. Tests pin Apify with `PLATFORM_APIFY_STUB` (ignored in production) so no suite reads a real account.

## Tables, row menus, pagination (client)
- Every list table uses `components/DataTable.jsx` (Excel-style header sort/filter, chips, selection, pagination, mobile cards). Do not hand-roll a new table.
- Row "..." actions use `components/RowMenu.jsx` (portaled, flips near screen bottom, theme tokens only). Never draw a menu inside a table cell.
- Page size comes from `utils/pagination.js` `usePageSize(id, fallback)`: options 10/25/50/100, remembered per table. Lists default 10 (Stripe, DataTables, MUI); report results default 25.
- Dashboard/History layouts use `.rl-page-head`, `.rl-tabs`, `.rl-toolbar`, `.rl-metric-grid` classes in components.css. Shell content reserves 96px bottom padding for the help bubble.
- A tab hidden in Chrome never finishes the "Loading your workspace" fade; check `document.visibilityState` before calling it a bug.
- Finished report results (Reel/Profile) use `DataTable` via `resultColumns()` in ReportEngine.jsx; the live streaming table keeps its own markup (rows arrive while on screen). Both show creator + link in ONE "Link" column so the table fits 1280px with no horizontal scroll; `.rl-results-dt` CSS trims padding and hides idle filter icons.
- Server-paged admin logs (Sessions, Ledger) use the shared `Pagination` + `usePageSize` (default 50), not DataTable. Team seats are a short list (`rl-info-list`), and the public ClientPortal table stays hand-built (themed public page, popovers are portaled to body).
- ONE page-size default (10) for every table, in `utils/pagination.js`. Screens never pass their own default. Creators fetches 50 rows per request (`FETCH_CHUNK`) but shows the shared page size.
- Text must never leave its box: global `overflow-wrap: break-word` + `min-width: 0` on cards, `.rl-clip` (ellipsis + title) for single-line names. The smoke test "text stays inside its box" fails on any card or text that spills, and "phone width" fails on sideways page scroll at 390px.
- Admin "Features" override list is driven by one `FEATURES` array in admin/Clients.jsx; keep it equal to `FEATURE_KEYS` in features.service.js when adding a paid feature. Forcing Team seats on gives at least 5 seats.
- Filters are only offered where they can narrow something (DataTable `filterOk`): not on tables under 6 rows, not where every row has the same value, not on a text column when the screen has a search box. Filter icons are always visible (never hover-only). Give wide tables a `sticky: 'right'` actions column.
- Every list table in the app, admin included, is a DataTable (Clients, Ledger, Sessions, Usage, Cost Monitor, Help Insights, Scan history, report results, History, compare, portal). Deliberate exceptions: the credit-audit statement (grouped headers), the credit-calculation panel, and short label/value lists.
- Client portal (`/portal/:token`): public response now carries `reports` (one summary per report) and each row a `reportKey`, so the page can show per-report totals, tabs, filters, pagination and a CSV download. It sets `data-theme` on <html> so popovers follow its own light/dark toggle.
- `Collapsible` (components/Collapsible.jsx) is for secondary detail only (report Highlights, portal per-report breakdown). Never collapse primary content by default.
- `npm run audit:ui` sweeps every page at 1280 and 390 wide, light and dark, saves screenshots to `audit-shots/` and fails on overflow or sideways scroll. Run it before any UI push and LOOK at the screenshots; the detector cannot judge design.

## Performance architecture (read before touching startup, static serving or the job engine)
Measure with `scripts/perf/*` (bench-api, bench-pages, bench-apify*, compare); the last report is `perf-results/PERFORMANCE_REPORT.md`.
- **Static first.** `server/middleware/clientStatic.js` is mounted BEFORE body parsing and the session, so a page or asset never waits on a database trip. Never put anything that touches the database ahead of it. Hashed `/assets/*` and `/fonts/*` are cached a year (immutable); `index.html` and `landing.html` are `no-cache`.
- **Precompressed.** `client/scripts/postbuild.mjs` runs after `vite build`: prerenders the landing page, then writes `.br` and `.gz` copies. `npm run build` does it; `regress.js` has a BUILD step for it too. API JSON is compressed by `compression` on `/api` only.
- **Landing is prerendered** (`dist/landing.html`, sent for `/`), critical CSS inlined, React hydrates it (`main.jsx`). Browser-only values (saved theme, signed-in user) must not change the FIRST render or hydration mismatches; use a `mounted` flag (see Landing.jsx). Calls to action are real `<a href>` (`CtaLink`).
- **Started before the bundle:** `index.html` inline scripts fetch `/api/auth/me` and, per route, a page's first data (`window.__early`, consumed once within 15 s by `api/client.js`). The URLs there must match what the page requests EXACTLY. `vite.config.js` `routePreload()` preloads the route's chunk and CSS.
- **Not on the critical path:** HelpBot and DemoGuide are lazy and load when idle; nav chunks prefetch on hover and when idle (`utils/prefetch.js`).
- **Fonts** are self-hosted in `client/public/fonts` (latin + latin-ext, unicode-range, swap), inlined `@font-face`, two preloaded. No Google Fonts.
- **Database:** `requireLogin` skips a second lookup and overlapping identical lookups share one round trip (`coalesce`); session reads are coalesced (`coalescingStore.js`); plans are cached 20 s and cleared on admin save; the Mongo client compresses traffic (`zlib`). `/jobs` never loads report rows; `/campaigns` sums rows inside the database (`loadJobSummaries`).
- **Job engine:** reel links run in waves (`REEL_BATCH_SIZE` 20 x `REEL_CONCURRENCY` 3), per-link ledger/charge/creator writes run 8 at a time while row commits stay sequential and in order, and Apify calls long-poll (`waitForFinish=60`). The exact cost is settled AFTER the results are recorded (`deferCost`, `settleBatchCost`): the ledger row is an estimate for a few seconds, then becomes measured.
